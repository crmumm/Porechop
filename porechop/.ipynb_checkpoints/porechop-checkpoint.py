...
from .adapters import make_full_native_barcode_adapter,\
...
def main():
    ...
    matching_sets = find_matching_adapter_sets(check_reads, args.verbosity, args.end_size,
                                               ...
                                               args.adapter_threshold, args.threads, ADAPTERS)
    ...
    display_adapter_set_results(matching_sets, args.verbosity, args.print_dest, ADAPTERS)
    ...
    output_reads(reads, args.format, args.output, read_type, args.verbosity,
                 ...
                 args.discard_unassigned, matching_sets, ADAPTERS)
...
def find_matching_adapter_sets(check_reads, verbosity, end_size, scoring_scheme_vals, print_dest,
    ...
    #printList = [a.start_sequence for a in ADAPTERS if '(full sequencce)' not in a.name] CM
    #print(printList) CM
...
def display_adapter_set_results(matching_sets, verbosity, print_dest, ADAPTERS):
    #print('Here: In Display adapter set results CM')
    
    if verbosity < 1:
        return
    table = [['Set', 'Barcode', 'start %ID', 'end %ID']]
    row_colours = {}
    matching_set_names = [x.name for x in matching_sets]
    search_adapters = [a for a in ADAPTERS if '(full sequence)' not in a.name]
    #CM
    for adapter_set in search_adapters:
        ...
        table.append([adapter_set.name, adapter_set.start_sequence, start_score, end_score])
        if adapter_set.name in matching_set_names:
            row_colours[len(table) - 1] = 'green'
            
    if verbosity > 0:
        #CM edit
        print_table(table, print_dest, alignments='LRRR', row_colour=row_colours,
                    fixed_col_widths=[30, 40, 10, 10])
...
def output_reads(reads, out_format, output, read_type, verbosity, discard_middle,
                 ...
                 untrimmed, threads, discard_unassigned, matching_sets, ADAPTERS):
    if verbosity > 0:
    ...
    if barcode_dir is not None:
        ...
        detailsOut.write(print_table(display_adapter_set_results(matching_sets = matching_sets,
                        ...
                        print_dest = print_dest, return_str=True, header_format = None,
                        alignments='LRRR', row_colour=None, fixed_col_widths=[30, 40, 10, 10]) + '\n')
